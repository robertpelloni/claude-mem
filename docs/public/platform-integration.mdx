...
Claude-Mem provides an official Gemini CLI extension via the Model Context Protocol (MCP).
...
npm install
npm run build
# Configure Gemini CLI to use the built MCP server
```

#### MCP Tools

The extension exposes the following tools to Gemini:

- `search_memory`: Search project history and past work.
- `get_recent_context`: Retrieve recent context for the current project.

#### Implementation

The extension acts as an MCP server that proxies requests to the main `claude-mem` worker service.
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "search_memory") {
    const response = await axios.get(`${WORKER_URL}/api/search`, { params: args });
