 ...
 * Settings are stored in ~/.borg-extension/settings.json
 */
...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  /**
   * Get environment settings (from ~/.borg-extension/settings.json)
   */
  ...
  /**
   * Update environment settings (in ~/.borg-extension/settings.json) with validation
   */
  private handleUpdateSettings = this.wrapHandler((req: Request, res: Response): void => {
    ...
    if (existsSync(settingsPath)) {
      ...
      try {
        ...
        res.status(500).json({
          success: false,
          error: 'Settings file is corrupted. Delete ~/.borg-extension/settings.json to reset.'
        });
        return;
      }
    }
