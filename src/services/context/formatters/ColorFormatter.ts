...
export function renderColorFooter(totalDiscoveryTokens: number, totalReadTokens: number): string[] {
  ...
  return [
    '',
    `${colors.dim}Access ${workTokensK}k tokens of past research & decisions for just ${totalReadTokens.toLocaleString()}t. Use the borg-extension skill to access memories by ID.${colors.reset}`
  ];
}
