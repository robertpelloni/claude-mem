...
interface HeaderProps {
  ...
  currentView: 'feed' | 'help' | 'status' | 'search' | 'dashboard' | 'pro' | 'graph';
  onViewChange: (view: 'feed' | 'help' | 'status' | 'search' | 'dashboard' | 'pro' | 'graph') => void;
  compact?: boolean;
}
...
export function Header({
  ...
  return (
    <div className={`header ${compact ? 'compact' : ''}`} style={compact ? { padding: '8px 12px', height: '48px' } : undefined}>
      <h1>
        <div style={{ position: 'relative', display: 'inline-block' }}>
          ...
          />
          {queueDepth > 0 && (
            <div className="queue-bubble" style={compact ? { fontSize: '8px', width: '12px', height: '12px', right: '-4px', top: '-4px' } : undefined}>
              {queueDepth}
            </div>
          )}
        </div>
      ...
      <div className="status" style={compact ? { gap: '8px' } : undefined}>
        {!compact && (
          <>
            <a
        ...
        <select
          ...
          style={compact ? { fontSize: '11px', padding: '2px 4px', width: '100px' } : undefined}
        >
        ...
        <button
          className={`settings-btn ${currentView === 'search' ? 'active' : ''}`}
        ...
        <button
          className={`settings-btn ${currentView === 'graph' ? 'active' : ''}`}
        ...
        <button
          className={`settings-btn ${currentView === 'help' ? 'active' : ''}`}
        ...
        <button
          className={`settings-btn ${currentView === 'feed' ? 'active' : ''}`}
