 ...
 * - CLI: `borg-extension generate` / `borg-extension clean`
...
const DB_PATH = path.join(os.homedir(), '.borg-extension', 'borg-extension.db');
...
function walkDirectoriesWithIgnore(dir: string, folders: Set<string>, depth: number = 0): void {
  ...
  const ignorePatterns = [
    ...
    '.borg-extension', '.open-next', '.turbo'
  ];
...
function formatObservationsForClaudeMd(observations: ObservationRow[], folderPath: string): string {
  ...
  lines.push('');
  lines.push('<!-- This section is auto-generated by borg-extension. Edit content outside the tags. -->');
  lines.push('');
 ...
 *
 * For each file with <borg-extension-context> tags:
  ...
  try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            try {
              ...
              if (content.includes('<borg-extension-context>')) {
    ...
    for (const file of filesToProcess) {
      ...
      try {
        ...
        const stripped = content.replace(/<borg-extension-context>[\s\S]*?<\/borg-extension-context>/g, '').trim();
