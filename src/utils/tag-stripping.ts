 ...
 * 1. <borg-extension-context> - System-level tag for auto-injected observations
...
function countTags(content: string): number {
  ...
  const contextCount = (content.match(/<borg-extension-context>/g) || []).length;
...
function stripTagsInternal(content: string): string {
  ...
  return content
    .replace(/<borg-extension-context>[\s\S]*?<\/borg-extension-context>/g, '')
