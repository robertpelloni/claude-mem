...
async function getDatabaseInfo(
  ...
  try {
    const dbPath = path.join(dataDir, "borg-extension.db");
...
export async function collectDiagnostics(
  ...
  const homeDir = os.homedir();
  const dataDir = path.join(homeDir, ".borg-extension");
  ...
  const isDevMode = cwd.includes("borg-extension") && !cwd.includes(".claude");

  // Collect version information
  const [borgExtension, claudeCode, bun, osVersion] = await Promise.all([
    getClaudememVersion(),
  ...
  const database = {
    path: sanitizePath(path.join(dataDir, "borg-extension.db")),
...
export function formatDiagnostics(diagnostics: SystemDiagnostics): string {
  ...
  output += "## Environment\n\n";
  output += `- **Claude-mem**: ${diagnostics.versions.borgExtension}\n`;
