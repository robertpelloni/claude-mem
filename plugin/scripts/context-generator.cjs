...
`).filter(n=>n.trim()),t="";for(let n=s.length-1;n>=0;n--)try{let i=s[n];if(!i.includes('"type":"assistant"'))continue;let d=JSON.parse(i);if(d.type==="assistant"&&d.message?.content&&Array.isArray(d.message.content)){let c="";for(let _ of d.message.content)_.type==="text"&&(c+=_.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){t=c;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return I.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:a},e),{userMessage:"",assistantMessage:""}}}async function es(a,e=!1){let s=Je(),t=a?.cwd??process.cwd(),n=Me(t),i=null;try{i=new q}catch(f){if(f.code==="ERR_DLOPEN_FAILED"){try{(0,j.unlinkSync)(qe)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw f}let d=Array.from(s.observationTypes),c=d.map(()=>"?").join(","),_=Array.from(s.observationConcepts),u=_.map(()=>"?").join(","),l=i.db.prepare(`
  ...
  `).all(n,...d,..._,s.totalObservationCount),g=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+Qe),b="",E="";if(s.showLastMessage&&l.length>0)try{let f=a?.session_id,N=l.find(R=>R.sdk_session_id!==f);if(N){let R=N.sdk_session_id,L=ze(t),w=Q.default.join((0,z.homedir)(),".claude","projects",L,`${R}.jsonl`),W=Ze(w);b=W.userMessage,E=W.assistantMessage}}catch{}if(l.length===0&&g.length===0)return i?.close(),e?`
