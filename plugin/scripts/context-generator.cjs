...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),u=l?`${l[1].split("/").pop()}:${l[2]}`:"unknown",T={...t,location:u};return this.warn(e,`[HAPPY-PATH] ${s}`,T,r),o}},L=new oe;var k=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,Se.join)((0,fe.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:ge,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:he,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,X.existsSync)(e))return this.getAllDefaults();let s=(0,X.readFileSync)(e,"utf-8"),t=JSON.parse(s),r=t;if(t.env&&typeof t.env=="object"){r=t.env;try{(0,X.writeFileSync)(e,JSON.stringify(r,null,2),"utf-8"),L.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){L.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let o={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))r[a]!==void 0&&(o[a]=r[a]);return o}};var Ve={};function Ye(){return typeof __dirname<"u"?__dirname:(0,N.dirname)((0,Re.fileURLToPath)(Ve.url))}var ms=Ye(),v=k.get("CLAUDE_MEM_DATA_DIR"),ie=process.env.CLAUDE_CONFIG_DIR||(0,N.join)((0,be.homedir)(),".claude"),Es=(0,N.join)(v,"archives"),Ts=(0,N.join)(v,"logs"),gs=(0,N.join)(v,"trash"),hs=(0,N.join)(v,"backups"),Ss=(0,N.join)(v,"settings.json"),Ne=(0,N.join)(v,"claude-mem.db"),fs=(0,N.join)(v,"vector-db"),bs=(0,N.join)(ie,"settings.json"),Os=(0,N.join)(ie,"commands"),Rs=(0,N.join)(ie,"CLAUDE.md");function Ae(c){(0,Oe.mkdirSync)(c,{recursive:!0})}var q=class{db;constructor(){Ae(v),this.db=new Le.Database(Ne),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable()}runMigration(e,s,t){try{if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(e))return;console.log(`[SessionStore] Applying migration ${e}: ${s}...`),this.db.transaction(()=>{t(),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(e,new Date().toISOString())})(),console.log(`[SessionStore] Migration ${e} (${s}) applied successfully`)}catch(r){throw console.error(`[SessionStore] Migration ${e} (${s}) failed:`,r.message),console.error(`[SessionStore] Rolled back migration ${e}`),r}}initializeSchema(){try{this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:o,type:a,concepts:d,files:p}=s,l=t==="date_asc"?"ASC":"DESC",u=r?`LIMIT ${r}`:"",T=e.map(()=>"?").join(","),g=[...e],m=[];if(o&&(m.push("project = ?"),g.push(o)),a)if(Array.isArray(a)){let n=a.map(()=>"?").join(",");m.push(`type IN (${n})`),g.push(...a)}else m.push("type = ?"),g.push(a);if(d){let n=Array.isArray(d)?d:[d],b=n.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...n),m.push(`(${b.join(" OR ")})`)}if(p){let n=Array.isArray(p)?p:[p],b=n.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");n.forEach(A=>{g.push(`%${A}%`,`%${A}%`)}),m.push(`(${b.join(" OR ")})`)}let O=m.length>0?`WHERE id IN (${T}) AND ${m.join(" AND ")}`:`WHERE id IN (${T})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${O}
    ...
    `).run(s.toISOString(),t,e)}markSessionFailed(e){let s=new Date,t=s.getTime();this.db.prepare(`
      ...
      WHERE id = ?
    `).run(s.toISOString(),t,e)}getSessionSummariesByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:o}=s,a=t==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",p=e.map(()=>"?").join(","),l=[...e],u=o?`WHERE id IN (${p}) AND project = ?`:`WHERE id IN (${p})`;return o&&l.push(o),this.db.prepare(`
      SELECT * FROM session_summaries
      ${u}
      ORDER BY created_at_epoch ${a}
      ${d}
    `).all(...l)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:r,project:o}=s,a=t==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",p=e.map(()=>"?").join(","),l=[...e],u=o?"AND s.project = ?":"";return o&&l.push(o),this.db.prepare(`
      SELECT
        up.*,
        s.project,
        s.sdk_session_id
      ...
      ${d}
    `).all(...l)}getTimelineAroundTimestamp(e,s=10,t=10,r){return this.getTimelineAroundObservation(null,e,s,t,r)}getTimelineAroundObservation(e,s,t=10,r=10,o){let a=o?"AND project = ?":"",d=o?[o]:[],p,l;if(e!==null){let m=`
    ...
    `).get(e)||null}getEndlessModeStats(){try{let e=this.db.prepare("SELECT SUM(discovery_tokens) as tokens, COUNT(*) as count FROM observations").get(),s=this.db.prepare("SELECT SUM(discovery_tokens) as tokens FROM session_summaries").get(),t=this.db.prepare("SELECT COUNT(*) as count FROM sdk_sessions").get(),r=this.db.prepare(`
      ...
      `),s=new Map;for(let d of e.all())try{let p=JSON.parse(d.files_modified);Array.isArray(p)&&p.forEach(l=>{let u=l.split("/").pop()||l;s.set(u,(s.get(u)||0)+1)})}catch{}let t=this.db.prepare(`
        SELECT concepts FROM observations WHERE concepts IS NOT NULL
      `),r=new Map;for(let d of t.all())try{let p=JSON.parse(d.concepts);Array.isArray(p)&&p.forEach(l=>{r.set(l,(r.get(l)||0)+1)})}catch{}let o=Array.from(s.entries()).map(([d,p])=>({name:d,count:p})).sort((d,p)=>p.count-d.count).slice(0,10),a=Array.from(r.entries()).map(([d,p])=>({name:d,count:p})).sort((d,p)=>p.count-d.count).slice(0,10);return{topFiles:o,topConcepts:a}}catch(e){return console.error("[SessionStore] Failed to get analytics:",e),{topFiles:[],topConcepts:[]}}}getKnowledgeGraph(e=50){let s=new Map,t=new Set;try{let r=this.db.prepare(`
      ...
      `).all(e);for(let o of r){let a=`session-${o.id}`;s.set(a,{id:a,label:`Session ${o.id}`,type:"session",val:8,data:o});let d=this.db.prepare(`
          ...
          FROM observations
          WHERE sdk_session_id = ?
        `).all(o.sdk_session_id);for(let l of d)if(l.concepts)try{let u=JSON.parse(l.concepts);if(Array.isArray(u))for(let T of u){let g=`concept-${T}`;s.has(g)||s.set(g,{id:g,label:T,type:"concept",val:5});let m=`${a}->${g}`;t.has(m)||t.add(JSON.stringify({source:a,target:g,type:"HAS_CONCEPT"}))}}catch{}let p=this.db.prepare(`
          SELECT files_read, files_modified
          FROM observations
          WHERE sdk_session_id = ?
        `).all(o.sdk_session_id);for(let l of p){let u=(T,g)=>{if(T)try{let m=JSON.parse(T);if(Array.isArray(m))for(let O of m){let E=`file-${O}`,n=O.split("/").pop()||O;if(!s.has(E))s.set(E,{id:E,label:n,type:"file",val:g==="MODIFIED"?6:3,path:O});else if(g==="MODIFIED"){let b=s.get(E);b.val=Math.max(b.val,6)}t.add(JSON.stringify({source:a,target:E,type:g}))}}catch{}};u(l.files_read,"READ"),u(l.files_modified,"MODIFIED")}}return{nodes:Array.from(s.values()),edges:Array.from(t).map(o=>JSON.parse(o))}}catch(r){return console.error("[SessionStore] Failed to get knowledge graph:",r),{nodes:[],edges:[]}}}close(){this.db.close()}importSdkSession(e){let s=this.db.prepare("SELECT id FROM sdk_sessions WHERE claude_session_id = ?").get(e.claude_session_id);return s?{imported:!1,id:s.id}:{imported:!0,id:this.db.prepare(`
      INSERT INTO sdk_sessions (
...
`).filter(r=>r.trim()),t="";for(let r=s.length-1;r>=0;r--)try{let o=s[r];if(!o.includes('"type":"assistant"'))continue;let a=JSON.parse(o);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let d="";for(let p of a.message.content)p.type==="text"&&(d+=p.text);if(d=d.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),d){t=d;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return L.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:c},e),{userMessage:"",assistantMessage:""}}}async function es(c,e=!1){let s=Je(),t=c?.cwd??process.cwd(),r=Me(t),o=null;try{o=new q}catch(b){if(b.code==="ERR_DLOPEN_FAILED"){try{(0,H.unlinkSync)(qe)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw b}let a=Array.from(s.observationTypes),d=a.map(()=>"?").join(","),p=Array.from(s.observationConcepts),l=p.map(()=>"?").join(","),u=o.db.prepare(`
  ...
  `).all(r,...a,...p,s.totalObservationCount),T=o.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(r,s.sessionCount+Qe),g="",m="";if(s.showLastMessage&&u.length>0)try{let b=c?.session_id,A=u.find(R=>R.sdk_session_id!==b);if(A){let R=A.sdk_session_id,I=ze(t),w=Q.default.join((0,z.homedir)(),".claude","projects",I,`${R}.jsonl`),W=Ze(w);g=W.userMessage,m=W.assistantMessage}}catch{}if(u.length===0&&T.length===0)return o?.close(),e?`
