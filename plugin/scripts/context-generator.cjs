...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=u?`${u[1].split("/").pop()}:${u[2]}`:"unknown",T={...t,location:l};return this.warn(e,`[HAPPY-PATH] ${s}`,T,n),o}},L=new oe;var k=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-sonnet-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_WORKER_HOST:"127.0.0.1",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,Se.join)((0,be.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:ge,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:he,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,X.existsSync)(e))return this.getAllDefaults();let s=(0,X.readFileSync)(e,"utf-8"),t=JSON.parse(s),n=t;if(t.env&&typeof t.env=="object"){n=t.env;try{(0,X.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),L.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){L.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let o={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(o[a]=n[a]);return o}};var Ve={};function Ye(){return typeof __dirname<"u"?__dirname:(0,N.dirname)((0,Re.fileURLToPath)(Ve.url))}var ms=Ye(),y=k.get("CLAUDE_MEM_DATA_DIR"),ie=process.env.CLAUDE_CONFIG_DIR||(0,N.join)((0,fe.homedir)(),".claude"),Es=(0,N.join)(y,"archives"),Ts=(0,N.join)(y,"logs"),gs=(0,N.join)(y,"trash"),hs=(0,N.join)(y,"backups"),Ss=(0,N.join)(y,"settings.json"),Ne=(0,N.join)(y,"claude-mem.db"),bs=(0,N.join)(y,"vector-db"),fs=(0,N.join)(ie,"settings.json"),Os=(0,N.join)(ie,"commands"),Rs=(0,N.join)(ie,"CLAUDE.md");function Ae(d){(0,Oe.mkdirSync)(d,{recursive:!0})}var q=class{db;constructor(){Ae(y),this.db=new Le.Database(Ne),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable()}initializeSchema(){try{this.db.run(`
    ...
    `).get(e)||null}getFilesForSession(e){let t=this.db.prepare(`
      ...
      FROM observations
      WHERE sdk_session_id = ?
    `).all(e),n=new Set,o=new Set;for(let a of t){if(a.files_read)try{let c=JSON.parse(a.files_read);Array.isArray(c)&&c.forEach(p=>n.add(p))}catch{}if(a.files_modified)try{let c=JSON.parse(a.files_modified);Array.isArray(c)&&c.forEach(p=>o.add(p))}catch{}}return{filesRead:Array.from(n),filesModified:Array.from(o)}}getSessionById(e){return this.db.prepare(`
    ...
    `).get(e)||(this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.type,t.title,t.subtitle,JSON.stringify(t.facts),t.narrative,JSON.stringify(t.concepts),JSON.stringify(t.files_read),JSON.stringify(t.files_modified),n||null,o,a.toISOString(),c);return{id:Number(T.lastInsertRowid),createdAtEpoch:c}}storeSummary(e,s,t,n,o=0){let a=new Date,c=a.getTime();this.db.prepare(`
      SELECT id FROM sdk_sessions WHERE sdk_session_id = ?
    `).get(e)||(this.db.prepare(`
        INSERT INTO sdk_sessions
        (claude_session_id, sdk_session_id, project, started_at, started_at_epoch, status)
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,s,t.request,t.investigated,t.learned,t.completed,t.next_steps,t.notes,n||null,o,a.toISOString(),c);return{id:Number(T.lastInsertRowid),createdAtEpoch:c}}markSessionCompleted(e){let s=new Date,t=s.getTime();this.db.prepare(`
      UPDATE sdk_sessions
    ...
    `).all(...u)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:n,project:o}=s,a=t==="date_asc"?"ASC":"DESC",c=n?`LIMIT ${n}`:"",p=e.map(()=>"?").join(","),u=[...e],l=o?"AND s.project = ?":"";return o&&u.push(o),this.db.prepare(`
      ...
      ${c}
    `).all(...u)}getTimelineAroundTimestamp(e,s=10,t=10,n){return this.getTimelineAroundObservation(null,e,s,t,n)}getTimelineAroundObservation(e,s,t=10,n=10,o){let a=o?"AND project = ?":"",c=o?[o]:[],p,u;if(e!==null){let m=`
    ...
    `).all(...e)}getSessionSummaryById(e){return this.db.prepare(`
      ...
      WHERE id = ?
      LIMIT 1
    `).get(e)||null}getEndlessModeStats(){try{let e=this.db.prepare("SELECT SUM(discovery_tokens) as tokens, COUNT(*) as count FROM observations").get(),s=this.db.prepare("SELECT SUM(discovery_tokens) as tokens FROM session_summaries").get(),t=this.db.prepare("SELECT COUNT(*) as count FROM sdk_sessions").get(),n=this.db.prepare(`
        SELECT
      ...
      `).get(),o=this.db.prepare(`
        ...
        FROM session_summaries
        WHERE discovery_tokens = 0
      `).get(),a=Math.floor(((n?.estimated_chars||0)+(o?.estimated_chars||0))/4);return{totalArchivedTokens:(e?.tokens||0)+(s?.tokens||0)+a,totalSessions:t?.count||0,totalObservations:e?.count||0}}catch(e){return console.error("[SessionStore] Failed to get endless mode stats:",e),{totalArchivedTokens:0,totalSessions:0,totalObservations:0}}}getKnowledgeGraph(e=50){let s=new Map,t=new Set;try{let n=this.db.prepare(`
        SELECT id, sdk_session_id, project, started_at_epoch
        FROM sdk_sessions
        WHERE status != 'failed'
        ORDER BY started_at_epoch DESC
        LIMIT ?
      `).all(e);for(let o of n){let a=`session-${o.id}`;s.set(a,{id:a,label:`Session ${o.id}`,type:"session",val:8,data:o});let c=this.db.prepare(`
          SELECT concepts
          FROM observations
          WHERE sdk_session_id = ?
        `).all(o.sdk_session_id);for(let u of c)if(u.concepts)try{let l=JSON.parse(u.concepts);if(Array.isArray(l))for(let T of l){let g=`concept-${T}`;s.has(g)||s.set(g,{id:g,label:T,type:"concept",val:5});let m=`${a}->${g}`;t.has(m)||t.add(JSON.stringify({source:a,target:g,type:"HAS_CONCEPT"}))}}catch{}let p=this.db.prepare(`
          SELECT files_read, files_modified
          FROM observations
          WHERE sdk_session_id = ?
        `).all(o.sdk_session_id);for(let u of p){let l=(T,g)=>{if(T)try{let m=JSON.parse(T);if(Array.isArray(m))for(let O of m){let E=`file-${O}`,r=O.split("/").pop()||O;if(!s.has(E))s.set(E,{id:E,label:r,type:"file",val:g==="MODIFIED"?6:3,path:O});else if(g==="MODIFIED"){let f=s.get(E);f.val=Math.max(f.val,6)}t.add(JSON.stringify({source:a,target:E,type:g}))}}catch{}};l(u.files_read,"READ"),l(u.files_modified,"MODIFIED")}}return{nodes:Array.from(s.values()),edges:Array.from(t).map(o=>JSON.parse(o))}}catch(n){return console.error("[SessionStore] Failed to get knowledge graph:",n),{nodes:[],edges:[]}}}close(){this.db.close()}importSdkSession(e){let s=this.db.prepare("SELECT id FROM sdk_sessions WHERE claude_session_id = ?").get(e.claude_session_id);return s?{imported:!1,id:s.id}:{imported:!0,id:this.db.prepare(`
      INSERT INTO sdk_sessions (
...
`).filter(n=>n.trim()),t="";for(let n=s.length-1;n>=0;n--)try{let o=s[n];if(!o.includes('"type":"assistant"'))continue;let a=JSON.parse(o);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let c="";for(let p of a.message.content)p.type==="text"&&(c+=p.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){t=c;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return L.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function es(d,e=!1){let s=Je(),t=d?.cwd??process.cwd(),n=Me(t),o=null;try{o=new q}catch(f){if(f.code==="ERR_DLOPEN_FAILED"){try{(0,H.unlinkSync)(qe)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw f}let a=Array.from(s.observationTypes),c=a.map(()=>"?").join(","),p=Array.from(s.observationConcepts),u=p.map(()=>"?").join(","),l=o.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,...a,...p,s.totalObservationCount),T=o.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+Qe),g="",m="";if(s.showLastMessage&&l.length>0)try{let f=d?.session_id,A=l.find(R=>R.sdk_session_id!==f);if(A){let R=A.sdk_session_id,I=ze(t),w=Q.default.join((0,z.homedir)(),".claude","projects",I,`${R}.jsonl`),j=Ze(w);g=j.userMessage,m=j.assistantMessage}}catch{}if(l.length===0&&T.length===0)return o?.close(),e?`
